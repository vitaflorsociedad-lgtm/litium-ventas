...
export default function PedidosClient({
  ...
  async function seleccionarCliente(clienteElegido: Cliente) {
    ...
    if (error) {
      setFacturas([]);
      setMensaje(
        `Cliente cargado, pero falló la lectura de facturas: ${error.message}`
      );
      return;
    }
  ...
  async function guardarPedido(estado: "Borrador" | "Enviado") {
    ...
    if (errorItems) {
      setGuardando(false);
      setMensaje(
        `Pedido creado, pero falló el detalle: ${errorItems.message}`
      );
      return;
    }
    ...
    if (estado === "Enviado") {
      try {
        ...
        if (!resp.ok) {
          ...
          setMensaje(
            `Pedido guardado pero falló el email: ${
    ...
    setMensaje(
      `Pedido ${
        estado === "Borrador" ? "guardado como borrador" : "enviado"
      } correctamente.`
    );
  }
